            ...
            if (token == XML.LT) {
                ...
                if (token instanceof Character) {
                    if (token == XML.SLASH) { // Close tag </
                        ...
                        c = x.next();
                        if (c == '-') {
                            if (x.next() == '-') x.skipPast("-->");
                            x.back();
                        } else if (c == '[') {
                            token = x.nextToken();
                            if (token.equals("CDATA") && x.next() == '[') {
                                if (ja != null) ja.put(x.nextCDATA());
                            ...
                            do {
                                ...
                                else if (token == XML.LT) i += 1;
                                else if (token == XML.GT) i -= 1;
                            } while (i > 0);
                        }
                    } else if (token == XML.QUEST) x.skipPast("?>"); // <?
                    else throw x.syntaxError("Misshaped tag"); // Open tag <
                } else {
                    ...
                    if (arrayForm) {
                        newja.put(tagName);
                        if (ja != null) ja.put(newja);
                    } else {
                        newjo.put("tagName", tagName);
                        if (ja != null) ja.put(newjo);
                    }
                    token = null;
                    for (;;) {
                        ...
                        if (token == XML.EQ) {
                            ...
                            token = null;
                        } else newjo.accumulate(attribute, "");
                    }
                    if (arrayForm && newjo.length() > 0) newja.put(newjo);
                    // Empty tag <.../>
                    if (token == XML.SLASH) {
                        ...
                        if (closeTag != null) {
                            ...
                            tagName = null;
                            if (!arrayForm && newja.length() > 0) newjo.put("childNodes", newja);
                            if (ja == null) {
                                if (arrayForm) return newja;
                                else           return newjo;
                            }
                ...
                if (ja != null) ja.put(token instanceof String ? XML.stringToValue((String)token) : token);
            }
        }
    }
